...
func filterHigher(d []sizeCheck, filterAmount int) []int {
	result := []int{}
...
func main() {
    ...
    total := sum(a)
    rootTotal := root.GetNodeSize()
    fmt.Println("over storage:", rootTotal > fsMax)
    fmt.Println("have enough?", rootTotal + requiredUnusedSpace < fsMax)

    required := (rootTotal + requiredUnusedSpace) - fsMax

    fmt.Println("reported total:", total)
    fmt.Println("/ total:", rootTotal)
    fmt.Println("required space:", required)

    // Filter out lower than required amount
    validChoices := filterHigher(r, required)
    sort.Ints(validChoices)
    answer := validChoices[0]
    // So that the amount is as low an impact as possible
    fmt.Println(answer)
}
