...
func executeInstructionPartTwo(count, from, to int, s *ship) {
	newStack := []string{}
	elem := []string{}
	...
	for idx, stack := range s.stacks {
		if stack.index == from {
			...
			newStack, elem = take(s.stacks[idx].contents, count)
			s.stacks[idx].contents = newStack
		}
	}
	...
	for idx, stack := range s.stacks {
		if stack.index == to {
			...
			result := place(s.stacks[idx].contents, elem)
			s.stacks[idx].contents = result
		}
	}
}
...
func take(stack []string, count int) ([]string, []string) {
	...
	taking := stack[len(stack)-count:]
	return stack[:len(stack)-count], taking
}
...
func main() {
	...
	for _, move := range moves {
		...
		executeInstructionPartTwo(a, f, t, &ship)
	}
