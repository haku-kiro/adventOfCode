...
func main() {
	...
	for _, line := range lines {
        ...
        if count == 3 {
            firstIntersection := intersection(lineA, lineB)
            secondIntersection := intersection(strings.Join(firstIntersection, ""), lineC)
            distinctSlice := distinct(secondIntersection)
            priority := countPriority(distinctSlice)
            result += priority
